...
(function($) {
	$.fn.fullpage = function(options) {
		...
		$('#form1').wrap('<div id="superContainer" />');
		...
		*/
		...
		function doneResizing() {
			...
			$('.section').each(function(){
				...
				if(options.verticalCentered){
					$(this).find('.tableCell').css('height', (windowsHeight - navHeight) + 'px');
				}
		...
		*/
		...
		function addTableClass(element){
			element.addClass('table').wrapInner('<article class="tableCell" style="height:' + (windowsHeight - navHeight) + 'px;" />');
		}
